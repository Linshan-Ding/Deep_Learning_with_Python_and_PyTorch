...
for step in range(EPOCH):
    ...
    print("周期：", step)
    # 可视化训练
    # viz.line([loss.item()], [step], win="loss", update='append')

# 测试模型并记录预测准确率
predict_days = abs(train_end)  # 预测的总天数
predict_right = 0  # 初始化预测涨跌正确天数
...
for i in range(n, len(df_all)):
    ...
    if i < test_index:
        ...
        if len(generate_data_test) >= 2:
            if (df_all_normal_tensor[i] >= df_all_normal_tensor[i - 1]) and (generate_data_test[-1] >= generate_data_test[-2]):
                predict_right += 1
            elif (df_all_normal_tensor[i] < df_all_normal_tensor[i - 1]) and (generate_data_test[-1] < generate_data_test[-2]):
                predict_right += 1
print("预测准确率：", predict_right / predict_days)
